...
import { transposeChord, analyzeHarmony, formatMusical, getRootNote, isMinorKey, getV7RootForMinorKey, getIiRootForMajorKey } from '../musicUtils.ts';
...
const ScaleDiagram: React.FC<{ intervals: string }> = ({ intervals }) => {
  ...
  return (
    <div className="flex flex-nowrap gap-3 mt-4 items-center overflow-x-auto pb-2">
      {pts.map((p, i) => (
        <div key={i} className="flex-shrink-0 flex items-center">
          <div className="flex flex-col items-center">
            <div className={`w-14 h-14 rounded-xl flex items-center justify-center text-xl font-bold border-2 transition-all shadow-md ${
          ...
          {i < pts.length - 1 && (
            <div className="mx-2 text-slate-700">
              <i className="fas fa-chevron-right text-base opacity-20"></i>
            </div>
          )}
        </div>
      ))}
...
const PracticeMode: React.FC<PracticeModeProps> = ({ tune, transposition }) => {
  ...
  const getScaleExample = (scaleName: string, currentTune: Tune, transposition: Transposition): { chord: string; context: string } => {
    if (!currentTune) return { chord: '', context: '' };

    const tuneRoot = getRootNote(currentTune.key); // Concert root of the tune
    const tuneIsMinor = isMinorKey(currentTune.key);
    ...
    switch (scaleName) {
      case 'Major Bebop':
        exampleChord = `${tuneRoot}maj7`; 
        context = `over Imaj7 in ${formatMusical(currentTune.key)}`;
        break;
      case 'Harmonic Minor':
      case 'Melodic Minor': 
        if (tuneIsMinor) {
          const v7Root = getV7RootForMinorKey(tuneRoot);
          exampleChord = `${v7Root}7alt`; 
          context = `over V7alt in ${formatMusical(currentTune.key)}`;
        } else {
          // Provide a generic dominant alt for a major key tune
          exampleChord = `C7alt`; 
          context = `over a V7alt chord`;
        }
        break;
      case 'Dorian':
        if (tuneIsMinor) {
          exampleChord = `${tuneRoot}m7`; 
          context = `over Im7 (or iim7 in major)`;
        } else {
          const iiRoot = getIiRootForMajorKey(tuneRoot);
          exampleChord = `${iiRoot}m7`; 
          context = `over iim7`;
        }
        break;
      default:
        exampleChord = tuneRoot;
        context = `in ${formatMusical(currentTune.key)}`;
        break;
    }
    return {
      chord: formatMusical(transposeChord(exampleChord, transposition)),
      context: context
    };
  };
    ...
    <div className="flex flex-col h-full bg-[#020617] font-sans relative overflow-hidden">
      <div className="bg-[#0f172a] border-b border-slate-800 px-6 py-4 flex flex-row justify-between items-center sticky top-0 z-50 gap-4 shadow-xl">
        <div className="flex items-center gap-4">
           ...
           <div>
            <h2 className="text-4xl font-jazz text-white leading-none truncate md:max-w-none uppercase">{tune.title}</h2>
            <div className="flex gap-3 text-sm font-black uppercase tracking-widest mt-1 text-slate-400">
              <span className="text-sky-400 font-realbook text-base lowercase">{tune.composer}</span>
        ...
        <div className="flex items-center gap-4">
          ...
          {tune.variants && tune.variants.length > 0 && (
            <div className="flex bg-black/40 rounded-xl p-1 border border-slate-700 h-10 hidden md:flex">
              <button
                ...
                className={`px-4 text-sm font-black uppercase tracking-widest rounded-lg transition-all ${
          ...
          <div className="bg-black/40 border border-slate-700 rounded-xl p-1 flex h-10">
            <button 
              ...
              className={`px-6 text-base font-black rounded-lg transition-all font-jazz tracking-widest ${activeTab === 'chords' ? 'bg-sky-500 text-black shadow-lg shadow-sky-500/20' : 'text-slate-400 hover:text-white'}`}
            >COMPANION</button>
            <button 
              onClick={() => setActiveTab('tech')}
              className={`px-6 text-base font-black rounded-lg transition-all font-jazz tracking-widest ${activeTab === 'tech' ? 'bg-sky-500 text-black shadow-lg shadow-sky-500/20' : 'text-slate-400 hover:text-white'}`}
            >TECHNIQUE</button>
          </div>
          <button 
          ...
          {activeTab === 'chords' ? (
                ...
                {isPlaying && (
                  <div className={`p-4 rounded-2xl border-2 flex justify-between items-center animate-in slide-in-from-top-2 duration-300 shadow-xl transition-all ${getActivePattern ? getPatternTheme(getActivePattern.type).bg : 'bg-slate-900/40 border-slate-800'}`}>
                     <div className="flex items-center gap-5">
                        ...
                        <div>
                          <h4 className="text-2xl font-jazz text-white leading-none">
                  ...
                  <div key={section.id} className="relative pt-10">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                      {section.measures.map((measure, mIdx) => {
                        ...
                        const chordFontSize = measure.chords.length > 1 ? 'text-3xl lg:text-4xl' : 'text-5xl lg:text-6xl';
                          ...
                          <div key={mIdx} className={`relative h-40 md:h-48 rounded-[2rem] border-2 transition-all duration-300 flex flex-col items-center justify-center overflow-hidden shadow-lg ${isCurrentBar ? 'bg-sky-500 border-sky-300 shadow-[0_0_40px_rgba(14,165,233,0.3)] scale-105 z-20' : `${theme.bg} ${theme.border}`}`}>
                            ...
                            {isPatternStart && (
                               <div className="absolute top-3 left-6 z-20">
                                  <span className={`text-xs font-black uppercase tracking-[0.2em] px-2.5 py-1 rounded-full bg-black/80 border border-white/10 ${isCurrentBar ? 'text-black bg-white/20' : theme.text} shadow-xl backdrop-blur-sm`}>
                                    {theme.label} in {formatMusical(transposeChord(pattern.key, transposition))}
                                  </span>
                               </div>
                            )}
                            <div className="flex flex-col items-center justify-center gap-1.5 px-3 w-full relative z-0"> {/* Adjusted px-2 to px-3 */}
                              {measure.chords.map((chord, cIdx) => (
                                <div key={cIdx} className="w-full flex flex-col items-center">
                                  ...
                                  <span className={`${chordFontSize} font-realbook font-bold text-center w-full ${isCurrentBar ? 'text-black' : 'text-white'}`}>
                                    {formatMusical(transposeChord(chord.symbol, transposition))}
                                  </span>
                                  {chord.roman && !isCurrentBar && <span className="text-xs text-slate-500 font-black mt-1 uppercase tracking-tighter">{chord.roman}</span>}
                                </div>
                              ))}
                            </div>
                            {isCurrentBar && (
                               <div className="absolute bottom-4 flex gap-2">
                                 {[0, 1, 2, 3].map(beat => (
                                   <div key={beat} className={`w-2.5 h-2.5 rounded-full border border-black/20 ${Math.floor(currentBeat % 4) === beat ? 'bg-black scale-125 shadow-md' : 'bg-black/5'}`}></div>
                                 ))}
                               </div>
                            )}
                          </div>
              ...
              <div className="lg:col-span-1 space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
                <div className="bg-[#0f172a] border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl">
                   <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-800">
                      ...
                      <h5 className="text-base font-black uppercase tracking-widest text-slate-300">Shed Insights</h5>
                   </div>
                   <div className="space-y-6">
                      {tune.practiceTools?.soloingTips?.map((tip, i) => (
                        <div key={i} className="flex gap-4 group">
                           <div className="w-2 h-2 rounded-full bg-sky-500/40 mt-1.5 group-hover:bg-sky-500 transition-colors"></div>
                           <p className="text-lg font-realbook text-slate-400 leading-relaxed italic">{tip}</p>
                        </div>
                      ))}
                   </div>
                </div>
                ...
                {tune.practiceTools?.recommendedLoops && (
                  <div className="bg-[#0f172a] border border-slate-800 rounded-[2.5rem] p-8 shadow-2xl">
                    <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-800">
                        ...
                        <h5 className="text-base font-black uppercase tracking-widest text-slate-300">Target Loops</h5>
                    </div>
                        ...
                        {tune.practiceTools.recommendedLoops.map((loop, i) => (
                          <div key={i} className="p-4 bg-black/40 border border-slate-800 rounded-2xl hover:border-sky-500/40 transition-all group">
                             <div className="flex justify-between items-center mb-1">
                                ...
                                <span className="text-xs text-slate-600 font-black">Bars {loop.bars[0]}-{loop.bars[1]}</span>
                             </div>
                             <p className="text-base text-slate-500 font-realbook italic group-hover:text-slate-300 transition-colors">{loop.focus}</p>
                          </div>
                        ))}
                    </div>
                  </div>
            ...
            <div className="animate-in fade-in duration-500 space-y-10">
               <div className="flex justify-between items-center">
                  ...
                  <span className="text-base text-sky-400 font-realbook bg-sky-500/10 px-4 py-1.5 rounded-full border border-sky-500/20 tracking-widest uppercase font-black">
               ...
               <div className="flex flex-col gap-8">
                  {SCALE_DATA.map((scale, i) => {
                    const scaleExample = getScaleExample(scale.name, tune, transposition);
                    return (
                      <div key={i} className="bg-[#0f172a] border border-slate-800 p-8 md:p-10 rounded-[3rem] hover:border-sky-500/30 transition-all group shadow-xl">
                        <div className="flex flex-col gap-6">
                            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                              <div>
                                <h4 className="text-4xl md:text-5xl font-jazz text-sky-400 mb-2 tracking-wide leading-none">{formatMusical(scale.name)}</h4>
                                <p className="text-xl font-realbook opacity-70">{formatMusical(scale.description)}</p>
                              </div>
                              <span className="text-sm font-black text-slate-700 uppercase tracking-[0.4em] border border-slate-800 px-4 py-1 rounded-full w-fit">Concept {i+1}</span>
                            </div>
                            <div className="w-full bg-black/30 p-8 rounded-[2.5rem] border border-slate-900 shadow-inner">
                              <span className="text-xs font-black text-slate-600 uppercase tracking-widest mb-4 block border-b border-slate-800/50 pb-2">Melodic Sequence</span>
                              <ScaleDiagram intervals={scale.intervals} />
                              <div className="mt-6 border-t border-slate-800/50 pt-6">
                                <span className="text-xs font-black text-slate-600 uppercase tracking-widest mb-2 block">Apply over:</span>
                                <p className="text-lg font-realbook text-sky-400">{scaleExample.chord} <span className="text-slate-500 text-base">({scaleExample.context})</span></p>
                              </div>
                            </div>
                        </div>
                      </div>
            ...
            <div className="flex flex-row justify-between items-center mb-10 relative z-10 gap-6">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-base text-slate-400 font-black uppercase tracking-widest mt-1">Harmonic Wisdom</p>
                </div>
              </div>
              <button onClick={loadAiSuggestions} disabled={isLoadingAi} className="px-8 py-4 bg-slate-900 hover:bg-slate-800 border-2 border-sky-500/40 text-sky-400 text-base font-black rounded-full transition-all uppercase tracking-widest shadow-xl active:scale-95">
            ...
            {aiSuggestions ? (
              <div className="relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-black/40 p-8 rounded-[2.5rem] border border-slate-800">
                   <p className="text-white text-2xl font-realbook leading-relaxed mb-8 border-l-4 border-sky-500/40 pl-6 italic">"{formatMusical(aiSuggestions.strategy)}"</p>
                   <div className="border-t border-slate-800 pt-8 flex flex-col md:flex-row gap-6 md:items-center">
                      <div className="bg-sky-500/10 border border-sky-500/20 px-4 py-2 rounded-xl text-center">
                         <span className="text-sm font-black text-sky-400 uppercase tracking-widest">The Drill</span>
                         <h4 className="text-xl font-jazz text-white mt-1">{formatMusical(aiSuggestions.drill.title)}</h4>
                      </div>
                      <p className="text-base text-slate-400 font-realbook leading-relaxed flex-1">{formatMusical(aiSuggestions.drill.description)}</p>
                   </div>
                </div>
              </div>
            ) : (
