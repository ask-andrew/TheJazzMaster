        ...
        <div className="max-w-6xl mx-auto pb-48">
          {activeTab === 'chords' ? (
            <div className="space-y-12 animate-in fade-in duration-500 relative">
              ...
              {/* Harmonic Context Engine Ribbon */}
              {isPlaying && (
                   ...
                   <div className="flex items-center gap-5">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-white/60 mt-1">
                          {getActivePattern?.type === 'minor-ii-V-i' ? 'Focus on ♭9 and ♭13 tensions' : 'Connect 3rds and 7ths'}
                        </p>
                      </div>
                   </div>
                   {getActivePattern && (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {section.measures.map((measure, mIdx) => {
                      ...
                      return (
                        <div 
                          key={mIdx}
                          className={`relative h-40 md:h-48 rounded-[2rem] border-2 transition-all duration-300 flex flex-col items-center justify-center overflow-hidden shadow-lg ${
                            isCurrentBar 
                              ? 'bg-sky-500 border-sky-300 shadow-[0_0_40px_rgba(14,165,233,0.3)] scale-105 z-20' 
                              : `${theme.bg} ${theme.border}`
                          }`}
                        >
                          {/* Color Side Track (Visual stitching to legend) */}
                          {pattern && (
                            <div className={`absolute left-0 top-0 bottom-0 w-2 ${theme.accent} opacity-80 shadow-[0_0_15px_rgba(0,0,0,0.5)] z-10`}></div>
                          )}

                          {/* Pattern Badge (Visible directly on card as requested) */}
                          {isPatternStart && (
                          ...
                          {isCurrentBar && (
                             <div className="absolute bottom-4 flex gap-2">
                               {[0, 1, 2, 3].map(beat => (
                                 <div 
                                   key={beat} 
                                   className={`w-2 h-2 rounded-full border border-black/20 ${Math.floor(currentBeat % 4) === beat ? 'bg-black scale-125 shadow-md' : 'bg-black/5'}`}
                                 ></div>
                               ))}
                             </div>
                          )}
              ...
              {/* Harmonic Key (Legend) */}
              <div className="fixed bottom-6 right-6 z-[60] bg-[#0f172a]/95 backdrop-blur-2xl p-4 rounded-[2.5rem] border border-slate-700 shadow-2xl animate-in fade-in slide-in-from-right-4 duration-500 max-w-[240px]">
                 ...
                 <div className="space-y-4">
                    <div className="flex items-center gap-4 group">
                       <div className="w-4 h-6 rounded-sm bg-sky-400 shadow-[0_0_10px_rgba(56,189,248,0.2)]"></div>
                       <div>
                          <p className="text-[10px] font-black text-white uppercase leading-none tracking-wide">Major ii-V-I</p>
                          <p className="text-[8px] text-slate-500 font-realbook mt-1">Dorian → Mixo → Ionian</p>
                       </div>
                    </div>
                    <div className="flex items-center gap-4 group">
                       <div className="w-4 h-6 rounded-sm bg-indigo-400 shadow-[0_0_10px_rgba(129,140,248,0.2)]"></div>
                       <div>
                          <p className="text-[10px] font-black text-white uppercase leading-none tracking-wide">Minor ii-V-i</p>
                          <p className="text-[8px] text-slate-500 font-realbook mt-1">Harmonic Minor / Alt</p>
                       </div>
                    </div>
                    <div className="flex items-center gap-4 group">
                       <div className="w-4 h-6 rounded-sm bg-blue-400 shadow-[0_0_10px_rgba(96,165,250,0.2)]"></div>
                       <div>
                          <p className="text-[10px] font-black text-white uppercase leading-none tracking-wide">Turnaround</p>
                          <p className="text-[8px] text-slate-500 font-realbook mt-1">Bebop Scales / Cycles</p>
                       </div>
                    </div>
                 </div>
              </div>
          ...
          <div className="bg-[#0f172a] border-4 border-slate-800 rounded-[3.5rem] p-10 mt-20 overflow-hidden relative shadow-2xl">
            ...
            <div className="flex flex-row justify-between items-center mb-10 relative z-10 gap-6">
              ...
              <button 
                ...
                className="px-8 py-4 bg-slate-900 hover:bg-slate-800 border-2 border-sky-500/40 text-sky-400 text-[10px] font-black rounded-full transition-all uppercase tracking-widest shadow-xl active:scale-95"
              >
            ...
            {aiSuggestions ? (
              <div className="relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-black/40 p-8 rounded-[2.5rem] border border-slate-800">
                   <p className="text-white text-xl font-realbook leading-relaxed mb-8 border-l-4 border-sky-500/40 pl-6 italic">
                      "{formatMusical(aiSuggestions.strategy)}"
                   </p>
